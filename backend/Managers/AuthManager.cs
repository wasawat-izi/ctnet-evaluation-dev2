...
namespace backend.Managers
{
    public class AuthManager : IAuthManager
    {
        ...
        public async Task<(AppUser, string)> SignInUserAsync(string identifier, string password)
        {
            AppUser user = null;

            if (identifier.Contains("@"))
            {
                user = await _repository.GetUserByEmailAsync(identifier);
            }
            else
            {
                user = await _repository.GetUserByUsernameAsync(identifier);
            }

            if(user == null)
                throw new Exception("Invalid email/username or incorrect password");
